...
app/
  ...
  components/
    ...
    Footer.tsx
    ClientsMarquee.tsx          # Single-row infinite marquee — bigger logos (h-9/h-10) at white/70 idle, full brand color + drop-shadow on hover. Edge-fade mask. py-4 md:py-6 outer.
...
6. **ClientsMarquee** — Bigger, more visible logos in a single-row infinite marquee. Demoted from the original §2 slot. Wrapper is tight `py-4 md:py-6` (no extra divider). Logo + name pairs at `h-9 md:h-10` SVGs and `text-base md:text-lg` names, white/70 idle, full brand color + drop-shadow halo on hover. Edge-fade mask via `.tech-marquee-mask`. Sits between Process (§5) and Featured Product (§7) — proof point that bridges the *how we work* sections into the *what we ship* sections.
...
*(None currently locked — ClientsMarquee was unlocked when logos were rescaled to be more visible.)*
